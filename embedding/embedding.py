...
for M, N, K in MNKs:
    ...
    run_benchmark(lib.embedding_fp32, i, weight, "fp32", o)
    run_benchmark(partial(embedding), i, weight, "fp32_th")

    print("-" * 110)
    weight_f16 = torch.randn((M, K)).half().cuda().contiguous()
    o_f16 = torch.zeros((N, K)).half().cuda().contiguous()
    run_benchmark(lib.embedding_fp16, i, weight_f16, "fp16", o_f16)
    run_benchmark(partial(embedding), i, weight_f16, "fp16_th")
    print("-" * 110)